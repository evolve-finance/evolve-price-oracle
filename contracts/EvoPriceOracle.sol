...
contract EvoPriceOracle is IEvoPriceOracle {
    ...
    function toUint224(uint256 input) internal pure returns (uint224) {
        require(input <= uint224(-1), "EvoPriceOracle: UINT224_OVERFLOW");
    ...
    function initialize(address uniswapV2Pair) external {
        ...
        require(
            ...
            "EvoPriceOracle: ALREADY_INITIALIZED"
        );
        ...
        returns (uint224 price, uint32 T)
    {
        ...
        require(pair.initialized, "EvoPriceOracle: NOT_INITIALIZED");
        Pair storage pairStorage = getPair[uniswapV2Pair];
        ...
        require(T >= MIN_T, "EvoPriceOracle: NOT_READY"); //reverts only if the pair has just been initialized
